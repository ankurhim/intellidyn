...
pub struct CreateHTMaterialRequest {
    pub ht_date: String,
...
impl CreateHTMaterialRequest {
    pub async fn create_ht_material_table(
        Extension(service): Extension<Arc<DbService>>
    ) -> Json<Value> {

        let service_response = service.client
        .execute(
            "CREATE TABLE IF NOT EXISTS mwspl_ht_material_table (
                id SERIAL NOT NULL,
                ht_material_pk TEXT NOT NULL PRIMARY KEY,
    ...
    pub async fn drop_ht_material_table(
        Extension(service): Extension<Arc<DbService>>
    ) -> Json<Value> {

        let service_response = service.client
        .execute(
            "DROP TABLE IF EXISTS mwspl_ht_material_table;",
            &[]
        )
        .await
        .map(|val| Json(json!(val)))
    ...
    pub async fn truncate_ht_material_table(
        ...
        let service_response = service.client
        .execute(
            "TRUNCATE TABLE mwspl_ht_material_table;",
            &[]
        )
        .await
        .map(|val| Json(json!(val)))
    ...
    pub async fn create_new_ht_material(
        Path((user, login_key)): Path<(String, String)>,
        Extension(service): Extension<Arc<DbService>>,
        ...
        let ht_date = NaiveDate::parse_from_str(&payload.ht_date, "%d-%m-%Y").expect("PO Date parsing error");

        let service_response = service.client
        .execute(
            "INSERT INTO mwspl_ht_material_table (
                ht_material_pk,
                ht_date,
                drawing_no,
                available_qty,
