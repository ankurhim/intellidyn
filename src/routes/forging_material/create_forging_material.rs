...
pub struct CreateForgingMaterialRequest {
    pub forging_date: String,
...
impl CreateForgingMaterialRequest {
    pub async fn create_forging_material_table(
        Extension(service): Extension<Arc<DbService>>
    ) -> Json<Value> {

        let service_response = service.client
        .execute(
            "CREATE TABLE IF NOT EXISTS mwspl_forging_material_table (
                id SERIAL NOT NULL,
                forging_material_pk TEXT NOT NULL PRIMARY KEY,
                forging_date DATE NOT NULL,
    ...
    pub async fn drop_forging_material_table(
        Extension(service): Extension<Arc<DbService>>
    ) -> Json<Value> {

        let service_response = service.client
        .execute(
            "DROP TABLE IF EXISTS mwspl_forging_material_table;",
            &[]
        )
        .await
        .map(|val| Json(json!(val)))
    ...
    pub async fn truncate_forging_material_table(
        ...
        let service_response = service.client
        .execute(
            "TRUNCATE TABLE mwspl_forging_material_table;",
            &[]
        )
        .await
        .map(|val| Json(json!(val)))
    ...
    pub async fn create_new_forging_material(
        Path((user, login_key)): Path<(String, String)>,
        Extension(service): Extension<Arc<DbService>>,
        ...
        let forging_date = NaiveDate::parse_from_str(&payload.forging_date, "%d-%m-%Y").expect("PO Date parsing error");

        let service_response = service.client
        .execute(
            "INSERT INTO mwspl_forging_material_table (
                forging_material_pk,
                forging_date,
                drawing_no,
                available_qty,
