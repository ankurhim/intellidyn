...
use crate::routes::cutting_material::cutting_material_model::CuttingMaterial;

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FindCuttingMaterialRequest {
    pub drawing_no: Option<String>,
}

impl FindCuttingMaterialRequest {
    pub async fn find_cutting_material(
        ...
        .query(
            "SELECT * FROM mwspl_cutting_material_table WHERE batch_status = 'NOT ISSUED' ORDER BY cutting_date;",
            &[]
        )
        .await
        .map_err(|e| Json(json!(e.to_string())));
    ...
    pub async fn find_cutting_material_by_dwg_no(
        Path((user, login_key)): Path<(String, String)>,
        Extension(service): Extension<Arc<DbService>>,
        Query(query): Query<Self>,
    ) -> Json<Value> {
        ...
        let service_resp = service.client
        .query(
            "SELECT * FROM mwspl_cutting_material_table WHERE drawing_no = $1 AND batch_status = 'NOT ISSUED' ORDER BY cutting_date;",
            &[&query.drawing_no]
        )
        .await
        .map_err(|e| Json(json!(e.to_string())));
...
fn get_list(row_vector: Vec<Row>) -> Json<Value> {
    ...
    for row in row_vector {
        vector.push(CuttingMaterial {
            cutting_material_pk: Uuid::parse_str(row.get(1)).unwrap(),
